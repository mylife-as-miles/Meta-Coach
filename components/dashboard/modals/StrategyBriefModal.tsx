...
const StrategyBriefModal: React.FC<StrategyBriefModalProps> = ({ isOpen, onClose, match }) => {
    ...
    // Use fetched data, then match prop, then fallback to empty
    const displayData = {
        ...
        region: match?.tournament?.name || teamProfile?.region || 'TBD',
        format: match?.format || 'Bo1',
        matchTime: getLiveIn(match?.startTime) || 'Scheduled',
        headToHead: matchPrepData?.headToHead || { totalMeetings: 0, ourWins: 0, theirWins: 0 },
        // Empty arrays when no AI analysis available
        keyPlayers: matchPrepData?.keyPlayers || [],
        banSuggestions: matchPrepData?.banSuggestions || [],
        winConditions: matchPrepData?.winConditions || [],
        objectives: matchPrepData?.objectives || [],
    };
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        <div>
                            ...
                            {displayData.keyPlayers.length === 0 ? (
                                <div className="p-4 bg-surface-darker rounded-lg border border-white/5 text-center">
                                    <span className="text-gray-500 text-sm">AI analysis will appear after opponent data is loaded</span>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    {displayData.keyPlayers.map((player: any, idx: number) => (
                        ...
                        <div>
                            ...
                            {displayData.banSuggestions.length === 0 ? (
                                <div className="p-4 bg-surface-darker rounded-lg border border-white/5 text-center">
                                    <span className="text-gray-500 text-sm">No ban suggestions available</span>
                                </div>
                            ) : (
                                <div className="flex gap-3">
                                    {displayData.banSuggestions.map((champ: string, idx: number) => (
                        ...
                        <div>
                            ...
                            {displayData.winConditions.length === 0 ? (
                                <div className="p-4 bg-surface-darker rounded-lg border border-white/5 text-center">
                                    <span className="text-gray-500 text-sm">Win conditions will be generated by AI</span>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    {displayData.winConditions.map((condition: string, idx: number) => (
                        ...
                        <div>
                            ...
                            {displayData.objectives.length === 0 ? (
                                <div className="p-4 bg-surface-darker rounded-lg border border-white/5 text-center">
                                    <span className="text-gray-500 text-sm">Objectives will be analyzed from match data</span>
                                </div>
                            ) : (
                                <ul className="space-y-2">
                                    {displayData.objectives.map((obj: string, idx: number) => (
